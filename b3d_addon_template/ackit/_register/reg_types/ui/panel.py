...
class ACK_PanelFlags(Enum):
    ''' Use as decorator over a Panel subclass. '''
    HIDE_HEADER = auto()
    DEFAULT_CLOSED = auto()
    INSTANCED = auto()

    def __call__(self, deco_cls: 'ACK_Panel'):
        deco_cls.panel_flags.add(self)
        return deco_cls


class ACK_Panel(BaseUI, DrawExtension):
    label: str
    tab: str = GLOBALS.ADDON_MODULE
    context: str = ''
    space_type: str = 'VIEW_3D'
    region_type: str = 'UI'
    panel_flags: set[ACK_PanelFlags] = set()
    ...
    @classmethod
    def tag_register(deco_cls) -> 'ACK_Panel':
...
class ACK_PanelPopover(ACK_Panel):
    label: str
    tab: str = ''
    context: str = ''
    space_type: str = 'STATUSBAR'
    region_type: str = 'HEADER'
    panel_flags: set[ACK_PanelFlags] = {ACK_PanelFlags.INSTANCED}
